# Cielo

Reference summary derived from `documentations/providers/cielo.mdx`. Official page: <https://docs.malga.io/documentations/providers/cielo>.

Legend: **✓** supported by Malga · **✗** not supported by the provider · **—** offered by the provider but not yet wired up in Malga's API (contact suporte@malga.io to request).

A Cielo atua desde 1995, e é referência em processamento de pagamentos no Brasil. Oferece soluções completas para vendas online por cartão de crédito, disponibilizando recursos com segurança e eficiência para lojas gerenciarem e operarem de forma flexível.
[Mais informações.](https://desenvolvedores.cielo.com.br/api-portal/)

## Funções por métodos de pagamento suportados

| Serviços                               |                   Crédito                    |                     Pix                      |                    Boleto                    |                   Voucher                    |                Apple Pay                 |
| -------------------------------------- | :------------------------------------------: | :------------------------------------------: | :------------------------------------------: | :------------------------------------------: | :--------------------------------------: |
| Cobrança                               |   ✓   |   ✓   | — |    ✗    | ✓ |
| Pré-autorização                        |   ✓   |    ✗    |    ✗    |    ✗    | ✓ |
| Captura parcial                        |   ✓   |    ✗    |    ✗    |    ✗    | ✓ |
| Estorno parcial                        |   ✓   | — |    ✗    |    ✗    | ✓ |
| Estorno total                          |   ✓   | — |    ✗    |    ✗    | ✓ |
| Split                                  | — | — | — |    ✗    |  ✗  |
| Antifraude próprio                     | — |    ✗    |    ✗    |    ✗    |  ✗  |
| 3DS                                    | — |    ✗    |    ✗    |    ✗    |  ✗  |
| Token de Bandeira                      |   ✓   |    ✗    |    ✗    |    ✗    |  ✗  |
| Validação de cartão (zero dollar auth) |   ✓   |    ✗    |    ✗    |    ✗    |  ✗  |
| Flag de recorrência                    |   ✓   |    ✗    |    ✗    |    ✗    |  ✗  |
| Notificação de abertura de disputa     | — |    ✗    |    ✗    |    ✗    |  ✗  |
| Notificação de chargedback             | — |    ✗    |    ✗    |    ✗    |  ✗  |
| Suporte a moedas internacionais        | — | — | — | — |  ✗  |

 

Se você possui interesse em receber cotações e avaliar este provedor para o seu fluxo de pagamentos, entre em contato com o nosso time para avaliar;
