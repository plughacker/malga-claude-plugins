# Stripe

Reference summary derived from `documentations/providers/stripe.mdx`. Official page: <https://docs.malga.io/documentations/providers/stripe>.

Legend: **✓** supported by Malga · **✗** not supported by the provider · **—** offered by the provider but not yet wired up in Malga's API (contact suporte@malga.io to request).

A Stripe é uma plataforma de pagamentos global que opera no Brasil desde 2019, oferecendo infraestrutura completa para lojas processarem transações por cartão de crédito em diferentes países, além de métodos como o boleto. Referência internacional em segurança e robustez em pagamentos, possui uma ampla gama de serviços.
[Mais informações.](https://docs.stripe.com/)

## Funções por métodos de pagamento suportados

| Serviços                               |                   Crédito                    |                     Pix                      |                    Boleto                    |                 Voucher                  |                  Apple Pay                   |
| -------------------------------------- | :------------------------------------------: | :------------------------------------------: | :------------------------------------------: | :--------------------------------------: | :------------------------------------------: |
| Cobrança                               |   ✓   | — |   ✓   |  ✗  | — |
| Pré-autorização                        |   ✓   |    ✗    |    ✗    |  ✗  | — |
| Captura parcial                        |   ✓   |    ✗    |    ✗    |  ✗  | — |
| Estorno parcial                        |   ✓   | — |    ✗    |  ✗  | — |
| Estorno total                          |   ✓   | — |    ✗    |  ✗  | — |
| Split                                  | — | — | — |  ✗  |    ✗    |
| Antifraude próprio                     |   ✓   |    ✗    |    ✗    |  ✗  |    ✗    |
| 3DS                                    | — |    ✗    |    ✗    |  ✗  |    ✗    |
| Token de Bandeira                      |    ✗    |    ✗    |    ✗    |  ✗  |    ✗    |
| Validação de cartão (zero dollar auth) |    ✗    |    ✗    |    ✗    |  ✗  |    ✗    |
| Flag de recorrência                    | — |    ✗    |    ✗    |  ✗  |    ✗    |
| Notificação de abertura de disputa     |   ✓   |    ✗    |    ✗    |  ✗  |    ✗    |
| Notificação de chargedback             |   ✓   |    ✗    |    ✗    |  ✗  |    ✗    |
| Suporte a moedas internacionais        |   ✓   |   ✓   |   ✓   | ✓ |    ✗    |

 

Se você possui interesse em receber cotações e avaliar este provedor para o seu fluxo de pagamentos, entre em contato com o nosso time para avaliar;
